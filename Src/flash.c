...
//#define FLASH_SIZE ((__IO uint16_t *)((*(__IO uint16_t *)FLASHSIZE_BASE) * 1024))
...
HAL_StatusTypeDef flash_layout_append(uint16_t layout) { 
	...
	if (flash_layout_load_idx(idx) == layout) { return HAL_OK; }

	if (idx >= 0xFFFFU) {
		if ((s = flash_layout_erase_all()) != HAL_OK) { return s; }
		idx = 0;
	} else {
		++idx;
	}
